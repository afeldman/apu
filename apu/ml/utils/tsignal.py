...
This module provides functions for signal processing, such as converting between frequency and Mel scales, creating Mel filter banks, and applying perceptual transformations to input data.
...
Example:
    ...
    >>> transformed = perceptual_transform(x, mel_scales=[16, 32, 64], n_fft=512, sample_rate=44_100)
"""
...
def mel_filter_bank(
    ...
    for i in range(1, n_filters + 1):
        start, peak, end = int(bin_points[i - 1].item()), int(bin_points[i].item()), int(bin_points[i + 1].item())

        if peak > start:
            filter_bank[i - 1, start:peak] = (torch.arange(start, peak, dtype=torch.float32) - start) / (peak - start)

        if end > peak:
            filter_bank[i - 1, peak:end] = (end - torch.arange(peak, end, dtype=torch.float32)) / (end - peak)

    return filter_bank


def perceptual_transform(
    x: torch.Tensor, mel_scales=(16, 32, 64), n_fft=512, sample_rate=44_100, eps=1e-7
) -> torch.Tensor:
    """
